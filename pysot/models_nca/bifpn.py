...
class BIFPN(nn.Layer):
    def __init__(self,
        ...
        self.stack_bifpn_convs = nn.LayerList()
...
class BiFPNModule(nn.Layer):
    def __init__(self,
        ...
        self.eps = eps
        self.levels = levels
        self.bifpn_convs = nn.LayerList()
        # weighted
        # self.w1 = nn.Parameter(torch.Tensor(2, levels).fill_(init))
        self.w1 = paddle.create_parameter([2,levels], dtype='float32', default_initializer=paddle.nn.initializer.Constant(value=init))
        self.relu1 = nn.ReLU()
        # self.w2 = nn.Parameter(torch.Tensor(3, levels - 2).fill_(init))
        self.w2 = paddle.create_parameter([3, levels - 2], dtype='float32', default_initializer=paddle.nn.initializer.Constant(value=init))
        self.relu2 = nn.ReLU()
        for jj in range(2):
    ...
    def forward(self, inputs):
        ...
        w1 /= paddle.sum(w1, axis=0) + self.eps  # normalize
        w2 = self.relu2(self.w2)
        w2 /= paddle.sum(w2, axis=0) + self.eps  # normalize
        # build top-down
        idx_bifpn = 0
        pathtd = inputs
        inputs_clone = []
        ...
        for i in range(levels - 1, 0, -1):
            ...
            #     pathtd[i], scale_factor=1, mode='nearest'))/(w1[0, i-1] + w1[1, i-1] + self.eps)
        ...
        for i in range(0, levels - 2, 1):
            # pathtd[i + 1] = (w2[0, i] * pathtd[i + 1] + w2[1, i] * F.max_pool2d(pathtd[i], kernel_size=1) +
            ...
            # pathtd[i + 1] = self.bifpn_convs[idx_bifpn](pathtd[i + 1])
            # idx_bifpn = idx_bifpn + 1
            pathtd[i + 1] = (w2[0, i] * pathtd[i + 1] + w2[1, i] * pathtd[i] )/(w2[0, i] + w2[1, i] + self.eps)
            pathtd[i + 1] = self.bifpn_convs[idx_bifpn](pathtd[i + 1])
            idx_bifpn = idx_bifpn + 1
