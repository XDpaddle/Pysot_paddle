...
class ClassificationModel(nn.Layer):
    ...
    def forward(self, x):
        ...
        out1 = out.transpose([0, 2, 3, 1])
        batch_size, width, height, channels = out1.shape
        out2 = out1.reshape([batch_size, width, height,
...
class Anchors(nn.Layer):
    ...
    def forward(self, image):
        ...
        return paddle.to_tensor(all_anchors.astype(np.float32))
...
def conv_ws_2d(input,
               ...
               eps=1e-5):
    c_in = weight.shape[0]
    weight_flat = weight.reshape([c_in, -1])
    mean = weight_flat.mean(axis=1, keepdim=True).reshape([c_in, 1, 1, 1])
    std = weight_flat.std(axis=1, keepdim=True).reshape([c_in, 1, 1, 1])
