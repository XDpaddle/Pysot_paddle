...
def conv_bn(inp, oup, stride, padding=1):
    return nn.Sequential(
        nn.Conv2D(inp, oup, 3, stride, padding, bias_attr=False),
        nn.BatchNorm2D(oup),
        nn.ReLU6()
    )
...
class InvertedResidual(nn.Layer):
    def __init__(self, inp, oup, stride, expand_ratio, dilation=1):
        ...
        self.conv = nn.Sequential(
            # pw
            nn.Conv2D(inp, inp * expand_ratio, 1, 1, 0, bias_attr=False),
            ...
            nn.ReLU6(),
            # pw-linear
            nn.Conv2D(inp * expand_ratio, oup, 1, 1, 0, bias_attr=False),
            nn.BatchNorm2D(oup),
        )
...
class MobileNetV2(nn.Sequential):
    def __init__(self, width_mult=1.0, used_layers=[3, 5, 7]):
        ...
        self.add_sublayer('layer0', conv_bn(3, input_channel, 2, 0))
        ...
        for idx, (t, c, n, s, d) in \
            ...
            self.add_sublayer('layer%d' % (idx), nn.Sequential(*layers))
...
if __name__ == '__main__':
    ...
    tensor = paddle.rand([1,3,255,255])
    # net = net.cuda()
