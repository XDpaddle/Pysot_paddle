...
class BasicBlock(nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1,
        ...
        self.conv1 = nn.Conv2D(inplanes, planes,
                               stride=stride, dilation=dd, bias_attr=False,
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            if stride == 1 and dilation == 1:
                ...
                downsample = nn.Sequential(
                    nn.Conv2D(self.inplanes, planes * block.expansion,
                              kernel_size=3, stride=stride, bias_attr=False,
...
if __name__ == '__main__':
    kwargs = {'used_layers': [2, 3, 4]}  # 后加的
    net = resnet50(**kwargs)
    print(net)

    # var = torch.FloatTensor(1, 3, 127, 127).cuda()
    var = paddle.rand((1, 3, 127, 127), paddle.float32)
    # var = Variable(var)

    net(var)
    print('*************')
    # var = torch.FloatTensor(1, 3, 255, 255).cuda()
    var = paddle.rand((1, 3, 255, 255), paddle.float32)
    # var = Variable(var)
    net(var)
# export LD_LIBRARY_PATH=/home/keyuze/anaconda3/envs/pysot/lib