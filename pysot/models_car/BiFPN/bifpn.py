...
class BiFPNModule(nn.Layer):
    def __init__(self,
        ...
        self.w1 = paddle.create_parameter([2,levels], dtype=paddle.float32, default_initializer=paddle.nn.initializer.Constant(value=init))
        self.relu1 = nn.ReLU()
        # self.w2 = nn.Parameter(torch.Tensor(3, levels - 2).fill_(init))
        self.w2 = paddle.create_parameter([3, levels - 2], dtype=paddle.float32, default_initializer=paddle.nn.initializer.Constant(value=init))
        self.relu2 = nn.ReLU()
        for jj in range(2):
    ...
    def forward(self, inputs):
        ...
        for i in range(0, levels - 2, 1):
            ...
            pathtd[i + 1] = self.bifpn_convs[idx_bifpn](pathtd[i + 1])
            idx_bifpn = idx_bifpn + 1
        pathtd[levels - 1] = (w1[0, levels-1] * pathtd[levels - 1] + F.max_pool2d(
